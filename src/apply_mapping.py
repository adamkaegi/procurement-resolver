...
def _release(row: dict[str, str], config: dict[str, Any], raw_ref: str) -> dict[str, Any]:
    ...
    if source_id == "federal_contracts":
        ...
        amount, description, classification = _float(row, "contractAmount-montantContrat"), _value(row, "awardDescription-descriptionAttribution-eng") or _value(row, "title-titre-eng"), _value(row, "unspsc-unspsc") or _value(row, "gsin-nibs")
        start, end = _value(row, "contractStartDate-contratDateDebut"), _value(row, "contractEndDate-dateFinContrat")
    else:
        ocid, release_id, date = _value(row, "Vendor of Record (VOR) Number"), _value(row, "Vendor of Record (VOR) Number"), _value(row, "Estimated Contract Start Date")
    ...
    release = {"ocid": ocid, "id": release_id, "date": parse_date(date), "initiationType": "tender", "tag": ["award"], "buyer": {"name": buyer_name or "Unknown", "id": f"{source_id}:buyer", "jurisdiction": config["jurisdiction"]}, "awards": [{"id": release_id, "date": parse_date(date), "value": {"amount": amount, "currency": _value(row, "contractCurrency-contratMonnaie") or "CAD"}, "suppliers": [{"name": vendor or "Unknown", "id": None}], "description": description or None, "items": [{"classification": {"scheme": "GSIN" if len(classification) <= 6 else "UNSPSC", "id": classification or "UNKNOWN"}}], "contractPeriod": {"startDate": parse_date(start), "endDate": parse_date(end)}}, "contracts": [{"period": {"startDate": parse_date(start), "endDate": parse_date(end)}, "awardID": release_id}], "_provenance": {"source_id": source_id, "fetched_at": now, "mapping_version": config["mapping_version"], "raw_ref": raw_ref, "field_origins": field_origins, "extraction_conf": None}}
    return release
...
def ingest_csv(path: Path, config_path: Path, limit: int = 5000) -> list[dict[str, Any]]:
    ...
    with path.open(newline="", encoding="utf-8-sig") as stream:
        releases = [_release(row, config, str(path)) for row in list(csv.DictReader(stream))[:limit]]
